...
在“同步目标”页面可以进入某个目标站点的“导入渠道”页面，拉取目标站点中尚未通过远端 ID 关联到本站的现存渠道。勾选后导入会在本站创建对应渠道、模型记录和启用状态的同步关联；本地名称默认保留远端名称，如已重名会自动追加短后缀。new-api 的渠道列表接口通常不会返回上游 API Key，导入这类渠道时需要在导入页面手动补填 Key。
